'''
    Recv first then compute simulation version.
    Multiprocessing + Multithreading version.
...
# 注册共享类
SharedManager.register('Link', Link_manage.Link)
...
def Topological_sort(Graphmodule: fx.GraphModule) -> deque:
    """
    ...
    # Debug 信息
    print(f"[DEBUG][Topological_sort] Added {len(placeholder_nodes)} placeholders, "
          f"{len(call_nodes)} call nodes, {len(output_nodes)} outputs.")
    print(f"[DEBUG][Topological_sort] Total nodes sorted: {len(task_queue)}")
...
class Performance_Evaluation:
    ...
    def _Inference_schedule(self, Dist_IR: list[fx.GraphModule], Topology_Graph: nx.Graph, KV_Cache: bool) -> double:
        ...
        manager = mp.Manager()
        # local_time_list = manager.dict({i: 0.0 for i in range(self.Device_nums)})
        ...
        # Step 2. 主进程统一创建 Barrier（fork 继承）
        global_barriers = {}
        for key, group_size in all_comm_groups:
            if key not in global_barriers:
                global_barriers[key] = mp.Barrier(group_size)
                print(f"[Main] Created barrier for group={key} (size={group_size})")
        print(global_barriers)
        ...
        global_barriers[f"All-devices-sync"] = mp.Barrier(real_device_nums)

        # Step 4. 启动子进程（通过 fork 继承 barrier）
        processes = []
        ...
        #  Step 6. fork 模式下主动关闭 Manager，防止多轮 Evaluate() 残留
        try:
                    ...
                    # if recv_first[0] == True:
                        with global_lock:
                            if scheduler.can_compute() and compute_manage.check_compute_ready(task_node, mem_dict):
                                ...
                                tensor_meta = task_node.meta.get("tensor_meta", None)
                                if hasattr(tensor_meta, "shape"):
                                    tensor_life.shape = tensor_meta.shape
                                if hasattr(tensor_meta, "dtype"):
                                    tensor_life.dtype = str(tensor_meta.dtype)
...
def memory_thread(device_id, mem_dict, stop_event: Event) -> None:
    ...
    memory_manage.print_memory_tensor_report(device_id)
    memory_manage.print_memory_only_report(device_id)
    return None
...
def device_process(rank, 
    ...
    while sched_queue:
        ...
        if task.op == 'placeholder':
            if f'hccl_recv_' not in task.name:
                ...
                with lock:
                    tensor_life.start_time = local_time[0]
                    tensor_meta = task.meta.get("tensor_meta", None)
                    if hasattr(tensor_meta, "shape"):
                        tensor_life.shape = tensor_meta.shape
                    if hasattr(tensor_meta, "dtype"):
                        tensor_life.dtype = str(tensor_meta.dtype)
    ...
    compute_thread_obj = threading.Thread(target=compute_thread, args=(rank, compute_q, local_time, mem_dict, local_time_list, scheduler, lock, global_lock, recv_first))
    comm_thread_obj = threading.Thread(target=comm_thread, args=(rank, comm_q, local_time, mem_dict, local_time_list, link_manager, group_barriers, scheduler, pp_stage_gap, lock, global_lock, recv_first))
