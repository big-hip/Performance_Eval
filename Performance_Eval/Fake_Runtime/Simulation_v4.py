'''
    recv first, then compute, finally send last.
    Multiprocessing + Multithreading version.
    Multiprocessing check data ready. wait + ready
'''
...
from .Memory_manage import LifeTime_mp
        ...
        if not compute_q.empty():
            ...
            while True:
                ...
                if recv_first.is_set():
                    ...
                    with global_lock:
                        flag = False
                        if scheduler.can_compute() and compute_manage.check_compute_ready_mp(task_node, mem_dict):
                            ...
                            tensor_life = Memory_manage.LifeTime_mp()
                            tensor_life.start_time = local_time[0]
                            tensor_life.ready = True
                                ...
                                for inp in task_node.all_input_nodes:
                                    ...
                                    for succ in list(inp.users.keys()):
                                        if succ.name not in mem_dict:
                                            can_release = False
                                        elif isinstance(mem_dict[succ.name], Memory_manage.LifeTime_mp) and mem_dict[succ.name].wait:
                                            can_release = False
                                    if can_release:
...
def comm_thread(device_id: int,
    ...
    while True:
        ...
        while True:
            ...
            if task_node == 'Recv_placeholders' or link_manage.check_comm_data_ready_mp(task_node, mem_dict):  
                    ...
                    try:
                        ...
                        with global_lock:
                            ...
                            for name, life in mem_dict.items():
                                if isinstance(life, Memory_manage.LifeTime_mp) and life.wait:
                                    life.wait = False
...
def device_process(rank, 
    ...
    if comm_q.empty():
        comm_done.set()  # 没有通信任务则直接放行计算
    else:
        comm_done.clear()

    comp_done.set()

    compute_thread_obj = threading.Thread(target=compute_thread, args=(rank, compute_q, local_time, mem_dict, local_time_list, scheduler, lock, global_lock, recv_first, send_last, comm_done, comp_done, comm_q))
    comm_thread_obj = threading.Thread(target=comm_thread, args=(rank, comm_q, local_time, mem_dict, local_time_list, link_manager, group_barriers, scheduler, pp_stage_gap, lock, global_lock, recv_first, send_last, comm_done, comp_done, compute_q))
